...
void fetchQuestions(int amount, const string& difficulty, int category) {
    string readBuffer;
    CURL* curl;
    CURLcode res;
    ...
    if (curl) {
        string url = "https://opentdb.com/api.php?amount=" + to_string(amount)
                     ...
                     + "&category=" + to_string(category) + "&type=multiple";
    ...
    try {
        ...
        for (const auto& result : jsonData["results"]) {
            ...
            shuffle(begin(question.options), end(question.options), default_random_engine{});
...
Question fetchQuestion(int amount, const string& difficulty, int category) {
